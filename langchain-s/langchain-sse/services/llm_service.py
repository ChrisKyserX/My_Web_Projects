...
class AgentLLMService:
    ...
    最主要的功能是帮助用户创建工单，一个完整的创建工单流程如下：
    1.获取用户发送的消息，若识别为建单意图，则继续向下进行，若识别为其他意图或指定以下某个环节，则直接进行指定环节的工具调用
    2.获取token，并将token作为后续工具调用的基础
    3.获取工作表列表，让llm分析列表title与用户消息的意图并返回匹配的单个工作表对象，返回数据格式：{title,id}
    4.获取工作表的组件列表，让llm分析组件列表与用户消息的意图并返回匹配的组件数组，返回数据格式：[{id,value}]
    5.将llm处理好的组件列表作为入参，调用保存工单工具进行保存，并将结果返回给用户

    以上每一步都可以单独进行，可以与用户进行多轮对话进行，也可以直接进行整轮任务。
    正常情况只进行一轮，结果若失败也直接返回
